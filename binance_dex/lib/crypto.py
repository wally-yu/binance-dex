...
from binance_dex.lib.bech32 import convertbits, bech32_encode
...
""" most of functions referenced by: https://github.com/michailbrynard/ethereum-bip44-python, 
Changed address generation to use cosmos """


class ReMnemonic(Mnemonic):
    """
    Some how, when using official "Mnemonic" class from python v3.6 will report below issue:
    UnicodeDecodeError: 'ascii' codec can't decode byte 0xe3 in position 0: ordinal not in range(128)
    just make some simple fix to make it work
    """
...
class PublicKey(PublicKeyBase):
    ...
    def address(self, compressed=True):
        ...
        Returns:
            bytes: Base58Check encoded string
        """
        pubkey_hash160 = self.hash160()
        converted = convertbits(pubkey_hash160, 8, 5, True)  # hard-coded to "8", "5" as per cosmos sample
        prefix = 'tbnb' if IS_TEST_NET else 'bnb'
